...
{
	SDL_Scancode sdlScancode = WINRT_TranslateKeycode((int)args->Key, args->KeyStatus.ScanCode);
#if 0
	SDL_Keycode keycode = SDL_GetKeyFromScancode(sdlScancode);
	SDL_Log("key up, handled=%s, ext?=%s, released?=%s, menu key down?=%s, "
		"repeat count=%d, native scan code=0x%x, was down?=%s, vkey=%d, "
...
extern DECLSPEC void SDLCALL
WINRT_ProcessCharacterReceivedEventRawKeyCode(int rawKeyCode)
{
	wchar_t src_ucs2[2];
	char dest_utf8[16];
	int result;
