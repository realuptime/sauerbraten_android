...
WINRT_CreateWindow(_THIS, SDL_Window * window)
{
    ...
    */
    ...
    if (!(window->flags & SDL_WINDOW_OPENGL)) {
		...
		//else if (data->coreWindow.Get() != nullptr) {
		else if (data->coreWindow.Get() != nullptr || WINRT_XAMLPanel != nullptr) {
			Windows::Foundation::Collections::PropertySet^ properties = ref new Windows::Foundation::Collections::PropertySet();

			IInspectable *coreWindowAsIInspectable = nullptr;
			if (data->coreWindow.Get() != nullptr)
				coreWindowAsIInspectable = reinterpret_cast<IInspectable *>(data->coreWindow.Get());
			else
			{
				// Create a PropertySet and initialize with the EGLNativeWindowType (CoreWindow or SwapChainPanel).
				properties->Insert(ref new Platform::String(L"EGLNativeWindowTypeProperty"), reinterpret_cast<Platform::Object ^>((IInspectable *)WINRT_XAMLPanel));
				...
				float resolutionScale = (info->LogicalDpi / 96.0f) / GetDPIScale();
				properties->Insert(ref new Platform::String(L"EGLRenderResolutionScaleProperty"), PropertyValue::CreateSingle(resolutionScale));
				SDL_Log("SPI Scale: %f", GetDPIScale());
				coreWindowAsIInspectable = reinterpret_cast<IInspectable*>(properties);
			}

			struct EGLConfig
			{
				unsigned int renderTargetFormat;      // TODO(geofflang): remove this
				unsigned int depthStencilFormat;      // TODO(geofflang): remove this
			...
			} *hack = (EGLConfig *)_this->egl_data->egl_config;
			hack->alphaSize = 0;
			SDL_Log("WARNING: Angle hack for disabling alpha size enabled!");
...
{
	if (WINRT_XAMLWasEnabled) return;
#if NTDDI_VERSION >= NTDDI_WIN10
    SDL_WindowData * data = (SDL_WindowData *)window->driverdata;
    const Windows::Foundation::Size size(WINRT_PHYSICAL_PIXELS_TO_DIPS(window->w),
                                         WINRT_PHYSICAL_PIXELS_TO_DIPS(window->h));
