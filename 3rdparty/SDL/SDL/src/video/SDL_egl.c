...
SDL_EGL_ChooseConfig(_THIS) 
{
    ...
    for (i = 0; i < found_configs; i++ ) {
        ...
        for (j = 0; j < SDL_arraysize(attribs) - 1; j += 2) {
            ...
            if ( attribs[j+1] != EGL_DONT_CARE && (
				...
				if (attribs[j] == EGL_RED_SIZE)
				{
					SDL_Log("EGL[%d] EGL_RED_SIZE:%d", i, value);
				}
				else if (attribs[j] == EGL_GREEN_SIZE)
				{
					SDL_Log("EGL[%d] EGL_GREEN_SIZE:%d", i, value);
				}
				else if (attribs[j] == EGL_BLUE_SIZE)
				{
					SDL_Log("EGL[%d] EGL_BLUE_SIZE:%d", i, value);
				}
				else if (attribs[j] == EGL_ALPHA_SIZE)
				{
					SDL_Log("EGL[%d] EGL_ALPHA_SIZE:%d", i, value);
				}
				else if (attribs[j] == EGL_DEPTH_SIZE)
				{
					SDL_Log("EGL[%d] EGL_DEPTH_SIZE:%d", i, value);
				}
				else if (attribs[j] == EGL_STENCIL_SIZE)
				{
					SDL_Log("EGL[%d] EGL_STENCIL_SIZE:%d", i, value);
				}
            }
        }
	...
	SDL_Log("EGL best config: %d", bestIdx);
