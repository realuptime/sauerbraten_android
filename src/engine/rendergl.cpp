    ...
    if (glversion < 300)
    {
        ...
        for(;;)
        {
            ...
            if(exts > ext)
            {
                ...
                //std::string s = std::string(ext, size_t(exts-ext));
                //logoutf("Ext: %s", s.c_str());
            }
        }
    }
...
void gl_checkextensions()
{
    ...
    if(glversion < 210) fatal("OpenGL 2.0 or greater is required!");
	...
	{
#ifdef __ANDROID__
		const char *pos = strrchr(glslstr, ' ');
		if (pos)
			glslstr += pos - glslstr;
		logoutf("GLSL version: '%s'", glslstr);
#endif // __ANDROID__
		if (my_sscanf(glslstr, " %u.%u", &glslmajorversion, &glslminorversion) == 2)
			glslversion = glslmajorversion * 100 + glslminorversion;
	}

    parseglexts();

    if(glslversion < 100) conoutf("ERROR: GLSL 1.00 or greater is required! cur:%d", glslversion);
    ...
    {
        hasTF = true;
        if(glversion < 300 && dbgexts) conoutf("Using GL_ARB_texture_float extension.");
        shadowmap = 1;
    ...
    {
        hasTRG = true;
        if(glversion < 300 && dbgexts) conoutf("Using GL_ARB_texture_rg extension.");
    }
#endif // HAVE_GLES
    ...
    if(glversion >= 300 || hasext("GL_ARB_framebuffer_object"))
    {
        ...
        if(glversion < 300 && dbgexts) conoutf("Using GL_ARB_framebuffer_object extension.");
    }
    else if(hasext("GL_EXT_framebuffer_object"))
    {
        hasFBO = true;
        if(dbgexts) conoutf("Using GL_EXT_framebuffer_object extension.");

        if(hasext("GL_EXT_framebuffer_blit"))
        {
            hasFBB = true;
            if(dbgexts) conoutf("Using GL_EXT_framebuffer_blit extension.");
        }

        if(hasext("GL_EXT_packed_depth_stencil") || hasext("GL_NV_packed_depth_stencil"))
        {
            hasDS = true;
            if(dbgexts) conoutf("Using GL_EXT_packed_depth_stencil extension.");
        }
    }
    ...
    {
        hasMBR = true;
        if(glversion < 300 && dbgexts) conoutf("Using GL_ARB_map_buffer_range.");
    }
    ...
    if(glversion >= 310 || hasext("GL_ARB_uniform_buffer_object"))
    {
        useubo = 1;
        hasUBO = true;
        if(glversion < 310 && dbgexts) conoutf("Using GL_ARB_uniform_buffer_object extension.");
    }

    if(glversion >= 300 || hasext("GL_ARB_vertex_array_object"))
    {
        hasVAO = true;
        if(glversion < 300 && dbgexts) conoutf("Using GL_ARB_vertex_array_object extension.");
    }
    else if(hasext("GL_APPLE_vertex_array_object"))
    {
        hasVAO = true;
        if(dbgexts) conoutf("Using GL_APPLE_vertex_array_object extension.");
    }

    if(glversion >= 330 || hasext("GL_ARB_texture_swizzle") || hasext("GL_EXT_texture_swizzle"))
    {
        hasTSW = true;
        if(glversion < 330 && dbgexts) conoutf("Using GL_ARB_texture_swizzle extension.");
    }
...
#endif
        if(dbgexts) conoutf("Using GL_EXT_texture_compression_s3tc extension.");
    }
    ...
    {
        hasS3TC = true;
        if(dbgexts) conoutf("Using GL_EXT_texture_compression_dxt1 extension.");
    }
    if(hasext("GL_3DFX_texture_compression_FXT1"))
    {
        hasFXT1 = true;
        if(mesa) usetexcompress = max(usetexcompress, 1);
        if(dbgexts) conoutf("Using GL_3DFX_texture_compression_FXT1.");
    }
    if(hasext("GL_EXT_texture_compression_latc"))
    {
        hasLATC = true;
        if(dbgexts) conoutf("Using GL_EXT_texture_compression_latc extension.");
    }
    if(glversion >= 300 || hasext("GL_ARB_texture_compression_rgtc") || hasext("GL_EXT_texture_compression_rgtc"))
    {
        hasRGTC = true;
        if(glversion < 300 && dbgexts) conoutf("Using GL_ARB_texture_compression_rgtc extension.");
    }
    ...
    if(hasext("GL_EXT_texture_filter_anisotropic"))
    {
        ...
        hwmaxaniso = val;
        hasAF = true;
        if(dbgexts) conoutf("Using GL_EXT_texture_filter_anisotropic extension.");
    }
...
void setcammatrix()
{
    ...
    if(!drawtex)
    {
        ...
        //conoutf("getcursorpos : %f %f", x, y);
        //conoutf("%f %f %f", camera1->yaw, camera1->roll, camera1->pitch);
        ...
        camdir = vec(worldpos).sub(camera1->o).normalize();
    }
}
...
bool isthirdperson() { return player_g!=camera1 || detachedcamera || reflecting; }
...
void mousemove(int dx, int dy)
{
    ...
    {
        setcammatrix(); // This is needed for phones
        return;
    }
    ...
    fixcamerarange();
    if(camera1!=player_g && !detachedcamera)
    {
        player_g->yaw = camera1->yaw;
        player_g->pitch = camera1->pitch;
    }
}
...
void recomputecamera()
{
    ...
    else
    {
        ...
        camera1 = &tempcamera;
        if(detachedcamera && shoulddetach) camera1->o = player_g->o;
        else
        {
            *camera1 = *player_g;
        ...
        if(game::collidecamera())
        {
            ...
            movecamera(camera1, dir, clamp(thirdpersondistance - camera1->o.dist(player_g->o), 0.0f, 1.0f), 0.1f);
            if(thirdpersonup)
            {
                vec pos = camera1->o;
...
void drawreflection(float z, bool refract, int fogdepth, const bvec &col)
{
    ...
    if(refracting && z>=0 && !isthirdperson() && fabs(camera1->o.z-z) <= 0.5f*(player_g->eyeheight + player_g->aboveeye))
    {
