...
void toggleedit(bool force)
{
    if(!force)
    {
        ...
        if(player_g->state!=CS_ALIVE && player_g->state!=CS_DEAD && player_g->state!=CS_EDITING) return; // do not allow dead players to edit to avoid state confusion
        if(!game::allowedittoggle()) return;         // not in most multiplayer modes
    }
    if(!(editmode = !editmode))
    {
        player_g->state = player_g->editstate;
        player_g->o.z -= player_g->eyeheight;       // entinmap wants feet pos
        entinmap(player_g);                       // find spawn closest to current floating pos
    }
    else
    {
        game::resetgamestate();
        player_g->editstate = player_g->state;
        player_g->state = CS_EDITING;
    }
    cancelsel();
    stoppaintblendmap();
    keyrepeat(editmode);
...
{
    if(!editmode) { if(msg) conoutf("operation only allowed in edit mode"); return true; }
    ...
    if(!viewable && msg) conoutf("selection not in view");
    return !viewable;
}
...
bool editmoveplane(const vec &o, const vec &ray, int d, float off, vec &handle, vec &dest, bool first)
{
    plane pl(d, off);
    float dist = 0.0f;
    if(!pl.rayintersect(player_g->o, ray, dist))
        return false;

    dest = vec(ray).mul(dist).add(player_g->o);
...
void rendereditcursor()
{
    ...
    else
    {
        ...
        wdist = rayent(player_g->o, camdir, 1e16f,
        ...
        if((havesel || dragging) && !passthroughsel && !hmapedit)     // now try selecting the selection
            if(rayboxintersect(vec(sel.o), vec(sel.s).mul(sel.grid), player_g->o, camdir, sdist, orient))
        ...
        {
            vec w = vec(camdir).mul(wdist+0.05f).add(player_g->o);
            if(!insideworld(w))
            {
    ...
    {
        if(hmapedit==1)
            gle::color(bvec(0, hmapsel ? 255 : 40, 0));
        else
...
{
    if(noedit()) return;
    if(a.empty()) { conoutf("nothing more to %s", op == EDIT_REDO ? "redo" : "undo"); return; }
...
void saveprefab(char *name)
{
    ...
    if(!f) { conoutf("could not write prefab to %s", filename); return; }
    prefabheader hdr;
    ...
    streambuf<uchar> s(f);
    if(!packblock(*b->copy, s)) { delete f; conoutf("could not pack prefab %s", filename); return; }
    delete f;
...
prefab *loadprefab(const char *name, bool msg = true)
{
   ...
   if(!f) { if(msg) conoutf("could not read prefab %s", filename); return NULL; }
   prefabheader hdr;
   if(f->read(&hdr, sizeof(hdr)) != sizeof(prefabheader) || memcmp(hdr.magic, "OEBR", 4)) { delete f; if(msg) conoutf("prefab %s has malformatted header", filename); return NULL; }
   lilswap(&hdr.version, 1);
   if(hdr.version != 0) { delete f; if(msg) conoutf("prefab %s uses unsupported version", filename); return NULL; }
   streambuf<uchar> s(f);
   block3 *copy = NULL;
   if(!unpackblock(copy, s)) { delete f; if(msg) conoutf("could not unpack prefab %s", filename); return NULL; }
   delete f;
...
static void renderprefab(prefab &p, const vec &o, float yaw, float pitch, float roll, float size, const vec &color)
{
    ...
    glDrawElements(GL_TRIANGLES, p.numtris*3, GL_UNSIGNED_SHORT, (ushort *)0);
...
void editmat(char *name, char *filtername)
{
    ...
    if(filtername[0])
    {
        ...
        if(filter < 0)
        {
            conoutf("unknown material \"%s\"", filtername);
            return;
        }
    }
    int id = -1;
