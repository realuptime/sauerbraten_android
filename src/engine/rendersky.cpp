...
void loadsky(const char *basename, Texture *texs[6])
{
    ...
    loopi(6)
    {
        ...
        if(texs[i]==notexture) conoutf("could not load side %s of sky texture %s", side, basename);
    }
}
...
namespace fogdome
{
    ...
    void draw()
    {
        ...
        glDrawElements(GL_TRIANGLES, numindices + fogdomecap*capindices, GL_UNSIGNED_SHORT, indices);
        xtraverts += numverts;
        glde++;
