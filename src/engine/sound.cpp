...
struct soundchannel
{
    ...
    soundchannel(int id_) : id(id_) { reset(); }
...
void setmusicvol(int musicvol_)
{
#ifndef _NO_SOUND
    if(nosound) return;
    if(music) Mix_VolumeMusic((musicvol_*MIX_MAX_VOLUME)/255);
#endif
}
...
void initsound()
{
    ...
    {
        nosound = true;
        if(sound) conoutf("sound init failed (SDL_mixer): %s", Mix_GetError());
        return;
    }
...
bool soundsample::load(const char *dir, bool msg)
{
    ...
    conoutf("failed to load sample: packages/sounds/%s", name);
    return false;
}
...
static struct soundtype
{
    ...
    soundtype(const char *dir_) : dir(dir_) {}
...
void mapsound(char *name, int *vol, int *maxuses) { intret(mapsounds.addsound(name, *vol, *maxuses < 0 ? 0 : std::max(1, *maxuses))); }
...
int playsound(int n, const vec *loc, extentity *ent, int flags, int loops, int fade, int chanid, int radius, int expire)
{
    ...
    if(!sounds.configs.inrange(n)) { conoutf("unregistered sound: %d", n); return -1; }
...
ICOMMAND(playsound, "i", (int *n), intret(playsound(*n)) );
ICOMMAND(playsoundname, "s", (char *name), intret(playsoundname(name)) );
...
void initmumble()
{
    ...
    if(VALID_MUMBLELINK) return;
    ...
    #ifdef WIN32
        mumblelink = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE,
        ...
        if(mumblelink)
        {
            ...
            if(mumbleinfo) wcsncpy(mumbleinfo->name, L"SDLGL2", 256);
        }
    #elif defined(_POSIX_SHARED_MEMORY_OBJECTS)
        ...
        if(mumblelink >= 0)
        {
            ...
            if(mumbleinfo != (MumbleInfo *)-1) wcsncpy(mumbleinfo->name, L"SDLGL2", 256);
        }
    #endif
    if(!VALID_MUMBLELINK) closemumble();
#else
    logoutf("ERROR: Mumble positional audio is not available on this platform.");
#endif
}
...
void updatemumble()
{
    ...
    mumbleinfo->pos = mumblevec(player_g->o, true);
    mumbleinfo->front = mumblevec(vec(player_g->yaw*RAD, player_g->pitch*RAD));
    mumbleinfo->top = mumblevec(vec(player_g->yaw*RAD, (player_g->pitch+90)*RAD));
#endif
}
