...
std::recursive_mutex *_sdlLogMutex = nullptr;
...
void logoutf(FileIO *file, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    if (!file)
    {
        logoutf("ERROR: log file is NULL!");
        logoutfv(fmt, args);
    }
    else
        writelogv(file, fmt, args);
    va_end(args);
}
...
{
    if (!_sdlLogMutex) return;
    std::lock_guard<std::recursive_mutex> lock(*_sdlLogMutex);
//#if defined(__ANDROID__) || defined(_UNIVERSAL_APP_)
    static char buf[SDL_MAX_LOG_MESSAGE];
    vformatstring(buf, fmt, args, sizeof(buf));
    if (strlen(buf) < SDL_MAX_LOG_MESSAGE) SDL_Log("%s", buf);
    printf("%s\n", buf);
//#endif // __ANDROID__
