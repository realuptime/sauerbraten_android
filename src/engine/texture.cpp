...
static GLenum textype(GLenum &component, GLenum &format)
{
    ...
    switch(component)
    {
        ...
        case GL_DEPTH24_STENCIL8_OES:
            if(!format) format = GL_DEPTH_STENCIL_OES;
            type = GL_UNSIGNED_INT_24_8_OES;
            logoutf("textype: we have GL_DEPTH_STENCIL_OES!");
            break;
#else
...
SDL_Surface *loadsurface(const char *name)
{
    ...
    const char *foundFile = findfile(name, "rb");
	FileIO io;
	if (!io.Open(foundFile, "rb")) return nullptr;
	if (!s)
	{
		s = IMG_Load_RW(io.RW(), false);
	}
    if (s)
    {
        LOGF("%s : Successfully opened image", foundFile);
    }
    else
        conoutf("%s : Failed to open image", foundFile);
...
static bool texturedata(ImageData &d, const char *tname, Slot::Tex *tex = NULL, bool msg = true, int *compress = NULL, int *wrap = NULL)
{
    ...
    if(!tname)
    {
        ...
        if(tex->name[0]=='<')
        {
            ...
            if(!file) { if(msg) conoutf("could not load texture packages/%s", tex->name); return false; }
            file++;
        }
        else file = tex->name;
    ...
    else if(tname[0]=='<')
    {
        cmds = tname;
        file = strrchr(tname, '>');
        if(!file) { if(msg) conoutf("could not load texture %s", tname); return false; }
        file++;
    }
    ...
    if(flen >= 4 && (!strcasecmp(file + flen - 4, ".dds") || (dds && !raw)))
    {
        ...
        {
            if(msg) conoutf("could not load texture %s", dfile);
            return false;
        }
    ...
    if(!d.data)
    {
        ...
        if(!s) { if(msg) conoutf("could not load texture %s", file); return false; }
        int bpp = s->format->BitsPerPixel;
        if(bpp%8 || !texformat(bpp/8)) { SDL_FreeSurface(s); conoutf("texture must be 8, 16, 24, or 32 bpp: %s", file); return false; }
        if(max(s->w, s->h) > (1<<12)) { SDL_FreeSurface(s); conoutf("texture size exceeded %dx%d pixels: %s", 1<<12, 1<<12, file); return false; }
        d.wrap(s);
    }
...
{
    if (!name || name[0] == 0) return notexture;
    string tname;
...
ICOMMAND(textureload, "siii", (char *name, int *clamp, int *mipit, int *msg), intret(textureload(name, *clamp, *mipit, *msg) != notexture));
...
void texture(char *type, char *name, int *rot, int *xoffset, int *yoffset, float *scale)
{
    ...
    s.loaded = false;
    s.texmask |= 1<<tnum;
    if(s.sts.length()>=8) conoutf("warning: too many textures in slot %d", slots.length()-1);
...
void autograss(char *name)
{
    ...
    Slot &s = *slots.last();
    DELETEA(s.autograss);
    s.autograss = name[0] ? newstring(makerelpath("media", name, NULL, "<premul>")) : NULL;
}
COMMAND(autograss, "s");
...
void texlayer(int *layer, char *name, int *mode, float *scale)
{
    ...
    s.layermaskname = name[0] ? newstring(path(makerelpath("media", name))) : NULL;
...
Texture *cubemaploadwildcard(Texture *t, const char *name, bool mipit, bool msg, bool transient = false)
{
    ...
    loopi(6)
    {
        ...
        if(s.w != s.h)
        {
            if(msg) conoutf("cubemap texture %s does not have square size", sname);
            return NULL;
        }
        ...
        {
            if(msg) conoutf("cubemap texture %s doesn't match other sides' format", sname);
            return NULL;
        }
...
ushort closestenvmap(int orient, const ivec &co, int size)
{
    ...
    loc[DIM_R[dim]] += size/2;
    loc[DIM_C[dim]] += size/2;
...
{
    if(!hasS3TC || usetexcompress <= 1) { conoutf("OpenGL driver does not support S3TC texture compression"); return; }
    ...
    t = textureload(cfile);
    if(t==notexture) { conoutf("failed loading %s", infile); return; }
    ...
    if(!compressed) { conoutf("failed compressing %s", infile); return; }
    int fourcc = 0;
    switch(format)
    {
        ...
        default:
            conoutf("failed compressing %s: unknown format: 0x%X", infile, format); break;
            return;
    }
    ...
    if(!f) { conoutf("failed writing to %s", outfile); return; }
    ...
    for(int lw = width, lh = height;;)
    {
        ...
        glGetCompressedTexImage(GL_TEXTURE_2D, d.dwMipMapCount++, dst);
        dst += size;
        if(max(lw, lh) <= 1) break;
        if(lw > 1) lw /= 2;
        if(lh > 1) lh /= 2;
...
void savepng(const char *filename, ImageData &image, bool flip)
{
    ...
    switch(image.bpp)
    {
        ...
        default: conoutf("failed saving png to %s", filename); return;
    }
    stream *f = openfile(filename, "wb");
    if(!f) { conoutf("could not write to %s", filename); return; }
...
void savetga(const char *filename, ImageData &image, bool flip)
{
    ...
    {
        case 3: case 4: break;
        default: conoutf("failed saving tga to %s", filename); return;
    }
