...
static void showglslinfo(GLenum type, GLuint obj, const char *name, const char **parts = NULL, int numparts = 0)
{
    ...
    if(length > 1)
    {
        conoutf("GLSL ERROR (%s:%s)", type == GL_VERTEX_SHADER ? "VS" : (type == GL_FRAGMENT_SHADER ? "FS" : "PROG"), name);
        {
            //FileIO *l = getlogfile();
            FileIO *l = nullptr;
            GLchar *log = new GLchar[length];
            if(type) glGetShaderInfoLog(obj, length, &length, log);
            else glGetProgramInfoLog(obj, length, &length, log);
			if (l) l->Printf("%s\n", log);
            logoutf("%s", log);
            ...
            loopi(numparts)
            {
                ...
                while(*part)
                {
                    ...
                    if(partlines)
                    {
                        snprintf(buf, sizeof(buf), "%d(%d): ", i, line - startline);
                        if (l) l->Printf("%d(%d): ", i, line - startline);
                        partString += buf;
                    }
                    else
                    {
                        snprintf(buf, sizeof(buf), "%d: ", line);
                        if (l) l->Printf("%d: ", line);
                        partString += buf;
                    }
                    if (l) l->Write((int8_t *)part, 1, next ? next - part + 1 : strlen(part));
                    partString += std::string(part, next ? next - part + 1 : strlen(part));
                    if(!next)
                    {
                        if (l) l->PutC('\n');
                        partString += '\n';
                        logoutf("%s", partString.c_str());
                        partString = "";
                        break;
                    }
                    part = next + 1;
...
void setupshaders()
{
    ...
    maxfsuniforms = val/4;
#else
    maxvsuniforms = 1024 / 4;
    maxfsuniforms = maxfsuniforms;
#endif
