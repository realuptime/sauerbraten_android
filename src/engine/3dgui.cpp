...
static std::vector<gui> guis2d, guis3d;
            ...
            case -5:
                if(isdown)
                {
                    if(windowhit->gui2d)
                    {
                        ...
                        for (size_t j = 0; j < guis2d.size()-1; ++j) *guis2d[j].savedorigin = *guis2d[j + 1].savedorigin;
                        *guis2d.back().savedorigin = origin;
                        if(guis2d.size() > 1)
                        {
                            if(camera1->o.dist(*guis2d.back().savedorigin) >= camera1->o.dist(*guis2d[0].savedorigin))
                                guis2d.back().savedorigin->sub(camdir);
                        }
                    }
...
{
    if(guis2d.size()) { x = cursorx; y = cursory; }
    else x = y = 0.5f;
}
...
void setcursorpos(int x, int y, bool checkCursor)
{
    // TODO
    //if (!checkCursor || hascursor())
    {
        if (!screenw || !screenh)
        {
            logoutf("ERROR: screenw or screenh is zero!");
            cursorx = cursory = 0.5f;
        }
        else
        {
            cursorx = clamp(x / (float)screenw, 0.0f, 1.0f);
            cursory = clamp(y / (float)screenh, 0.0f, 1.0f);
            logoutf("cursorx:%f cursory:%f", cursorx, cursory);
        }
    }
}

//0 - centre always; 1 - free cursor
VARP(freecursor, 0, 1, 1);
VARP(freeeditcursor, 0, 1, 1);

static inline int freecursormode()
{
...
{
    //if(!guis2d.size() || !hascursor) return false;
    ...
    cursorx = clamp(cursorx + dx * guisens / screenw, 0.f, 1.f);
    cursory = clamp(cursory + dy * guisens / screenh, 0.f, 1.f);

    if (freecursormode())
    {
        //world->SearchTarget(cursorx * float(screenw)/screenh, cursory); // test
        if (!mousePressed())
        {
    ...
    //logoutf("cx:%f cy:%f", cursorx, cursory);
    return true;
}
...
void g3d_addgui(g3d_callback *cb, vec &origin, int flags)
{
    ...
    if (gui2d) guis2d.push_back(gui()); else guis3d.push_back(gui());
    gui &g = (gui2d ? guis2d : guis3d).back();
    g.cb = cb;
    g.origin = origin;
...
bool g3d_windowhit(bool on, bool act)
{
    ...
    return (guis2d.size() && hascursor) || (windowhit && !windowhit->gui2d);
}
...
void g3d_render()
{
    ...
    layoutpass = true;
    for (auto &gui : guis2d) gui.draw();
    for (auto &gui : guis3d) gui.draw();
    layoutpass = false;

    if(guis2d.size() || guis3d.size())
    {
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    }
