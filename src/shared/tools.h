...
#include <direct.h>
#define my_vsnprintf vsnprintf_s
#define my_sscanf sscanf
#define my_localtime_r(x, y) localtime_s(y, x) // reverse
#define my_gmtime_r(x, y) gmtime_s(y, x) // reverse
#define my_chdir _chdir

//int c99_snprintf(char *outBuf, size_t size, const char *format, ...);
//#define my_snprintf c99_snprintf
...
#define my_localtime_r(x, y) (*y = *localtime(x))
//#define my_localtime_r(x, y) localtime_s(y, x) // reverse


//#define my_gmtime_r gmtime_r
#define my_gmtime_r(x, y) (*y = *gmtime(x))
//#define my_gmtime_r(x, y) gmtime_s(y, x) // reverse
...
static inline T clamp(T val, U min, U max)
{
    return std::max(T(min), std::min(val, T(max)));
}
...
{
    size_t slen = std::min(strlen(s)+1, len);
    memcpy(d, s, slen);
    d[slen-1] = 0;
    return d;
}
...
#define loopvrevk(v) for(int k = (v).length()-1; k>=0; k--)
...
struct databuf
{
    ...
    template<class U>
    databuf(T *buf_, U maxlen_) : buf(buf_), len(0), maxlen((int)maxlen_), flags(0) {}
...
struct stringslice
{
    ...
    int len;
    stringslice() {}
    stringslice(const char *str_, int len_) : str(str_), len(len_) {}
    stringslice(const char *str_, const char *end_) : str(str_), len(int(end_-str_)) {}
...
template <class T> struct vector
{
    ...
    {
        int olen = alen;
        if(!alen) alen = std::max(_VECTOR_MIN_SIZE, sz);
...
template<class H, class E, class K, class T> struct hashbase
{
    ...
    hashbase(int size_ = DEFAULTSIZE)
      : size(size_)
    {
        numelems = 0;
        chunks = NULL;
        ...
        for(chain **p = &chains[h], *c = chains[h]; c; p = &c->next, c = c->next)
        {
            if(htcmp(key, H::getkey(c->elem)))
            {
                *p = c->next;
                c->elem.~E();
                //E tmp;
                //memcpy(&c->elem, &tmp, sizeof(E));
                memset(&c->elem, 0, sizeof(E));
                c->next = unused;
                unused = c;
                numelems--;
                return true;
...
template<class T> struct hashset : hashbase<hashset<T>, T, T, T>
{
    ...
    hashset(int size_ = basetype::DEFAULTSIZE) : basetype(size_) {}
...
template<class T> struct hashnameset : hashbase<hashnameset<T>, T, const char *, T>
{
    ...
    hashnameset(int size_ = basetype::DEFAULTSIZE) : basetype(size_) {}
...
template<class K, class T> struct hashtable : hashbase<hashtable<K, T>, hashtableentry<K, T>, K, T>
{
    ...
    hashtable(int size_ = basetype::DEFAULTSIZE) : basetype(size_) {}
...
extern stream *openrawfile(const char *filename, const char *mode, FileLocation location = AssetsLocation);
extern stream *openzipfile(const char *filename, const char *mode, FileLocation location = AssetsLocation);
extern stream *openfile(const char *filename, const char *mode, FileLocation location = AssetsLocation);
extern stream *opentempfile(const char *filename, const char *mode, FileLocation location = AssetsLocation);
extern stream *opengzfile(const char *filename, const char *mode, FileLocation location = AssetsLocation, stream *file = NULL, int level = Z_BEST_COMPRESSION);
extern stream *openutf8file(const char *filename, const char *mode, FileLocation location = AssetsLocation, stream *file = NULL);
extern char *loadfile(const char *fn, size_t *size, FileLocation location = AssetsLocation, bool utf8 = true);
...
extern unsigned long rand_xorshf96();

template<class T>
inline T *strrnchr(T *buff, char ch, size_t len)
{
