...
struct filestream : stream
{
    ...
    bool open(const char *name, const char *mode, FileLocation location)
    {
        if(file.IsValid()) return false;
        file.Open(name, mode, location);
        return file.IsValid();
    }

    bool opentemp(const char *name, const char *mode, FileLocation location)
    {
        if(file.IsValid()) return false;
        file.Open(name, mode, location);
        return file.IsValid();
...
#endif*/
        return file.Seek(pos, whence) >= 0;
    }

    size_t read(void *buf, size_t len) { return file.Read((int8_t *)buf, 1, len); }
    size_t write(const void *buf, size_t len) { return file.Write((int8_t *)buf, 1, len); }
    bool flush() { return !file.Flush(); }
    int getchar() { return file.GetC(); }
    bool putchar(int c) { return file.PutC(c)!=(size_t)EOF; }
    bool getline(char *str, size_t len) { return file.GetS((int8_t *)str, len) > 0; }
    bool putstring(const char *str) { return file.Write((int8_t *)str, 1, strlen(str)) != (size_t)EOF; }
...
#ifdef WIN32
        int result = vsprintf(buf, fmt, v);
#else
        int result = vsnprintf(buf, sizeof(buf), fmt, v);
#endif
        if (result > 0)
            result = file.Write((int8_t *)buf, 1, result);
        else
            result = 0;
        va_end(v);
        return result;
...
struct gzstream : stream
{
    ...
    void readbuf(size_t size_ = BUFSIZE)
    {
        if(!zfile.avail_in) zfile.next_in = (Bytef *)buf;
        size_ = std::min(size_, size_t(&buf[BUFSIZE] - &zfile.next_in[zfile.avail_in]));
        size_t n = file->read(zfile.next_in + zfile.avail_in, size_);
        if(n > 0) zfile.avail_in += n;
    }

    uchar readbyte(size_t size_ = BUFSIZE)
    {
        if(!zfile.avail_in) readbuf(size_);
    ...
    size_t read(void *buf_, size_t len)
    {
        if(!reading || !buf_ || !len) return 0;
        zfile.next_out = (Bytef *)buf_;
        zfile.avail_out = len;
        while(zfile.avail_out > 0)
        {
            ...
            if(err == Z_STREAM_END) { crc = crc32(crc, (Bytef *)buf_, len - zfile.avail_out); finishreading(); stopreading(); return len - zfile.avail_out; }
            else if(err != Z_OK) { stopreading(); break; }
        }
        crc = crc32(crc, (Bytef *)buf_, len - zfile.avail_out);
    ...
    size_t write(const void *buf_, size_t len)
    {
        if(!writing || !buf_ || !len) return 0;
        zfile.next_in = (Bytef *)buf_;
        zfile.avail_in = len;
        ...
        crc = crc32(crc, (Bytef *)buf_, len - zfile.avail_in);
...
struct utf8stream : stream
{
    ...
    bool readbuf(size_t size_ = BUFSIZE)
    {
        ...
        size_t n = file->read(&buf[buflen], std::min(size_, BUFSIZE - buflen));
        if(n <= 0) return false;
        buflen += n;
...
stream *openrawfile(const char *filename, const char *mode, FileLocation location)
{
    // TODO: Consider location
    const char *found = findfile(filename, mode);
    if(!found) return NULL;
    filestream *file = new filestream;
    if(!file->open(found, mode, location)) { delete file; return NULL; }
    return file;
}

stream *openfile(const char *filename, const char *mode, FileLocation location)
{
    stream *s = openzipfile(filename, mode, location);
    if(s) return s;
    return openrawfile(filename, mode, location);
}

stream *opentempfile(const char *name, const char *mode, FileLocation location)
{
    // TODO: consider location
    const char *found = findfile(name, mode);
    filestream *file = new filestream;
    if(!file->opentemp(found ? found : name, mode, location)) { delete file; return NULL; }
    return file;
}

stream *opengzfile(const char *filename, const char *mode, FileLocation location, stream *file, int level)
{
    stream *source = file ? file : openfile(filename, mode, location);
...
stream *openutf8file(const char *filename, const char *mode, FileLocation location, stream *file)
{
    stream *source = file ? file : openfile(filename, mode, location);
    if(!source) return NULL;
...
char *loadfile(const char *fn, size_t *size, FileLocation location, bool utf8)
{
    stream *f = openfile(fn, "rb", location);
    if(!f) return NULL;
    size_t len = f->size();
...
void PrintGZError(int decRet)
{
    switch (decRet)
    {
    case Z_BUF_ERROR: // if dstLen is not large enough to fit the inflated data
        conoutf("Z_BUF_ERROR error");
        break;
    case Z_MEM_ERROR:
        conoutf("Z_MEM_ERROR error");
        break;
    case Z_DATA_ERROR:
        conoutf("Z_DATA_ERROR error");
        break;
    }
}

int gzdecompress(const void *src, int srcLen, void *dst, int dstLen)
{
    z_stream strm  = {0};
