...
namespace gle
{
    ...
    void drawquads(int offset, int count)
    {
        ...
        //lDrawRangeElements_(GL_TRIANGLES, offset*4, (offset + count)*4-1, count*6, GL_UNSIGNED_SHORT, (ushort *)0 + offset*6);
		glDrawElements(GL_TRIANGLES, count * 6, GL_UNSIGNED_SHORT, (ushort *)0 + offset * 6);
    }
...
#else
                for (int idx = 0; idx < multidrawstart.length(); ++idx)
                    glDrawArrays(primtype, multidrawstart.getbuf()[idx], multidrawcount.getbuf()[idx]);
#endif
